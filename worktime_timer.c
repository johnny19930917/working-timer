...
void worktime_timer(int hours)
{
    ...
    while (1)
    {
        // Calculate remaining time by real world time elapsed
        int remaining = (int)(clockout_time - time(NULL));
        ...
        // Update only remaining time changed
        if (remaining != prev_remaining)
        {
            prev_remaining = remaining;

            double percent = (double)(seconds - remaining) / seconds;
        ...
        // Control the while loop execution frequency
        Sleep(50);
    }
